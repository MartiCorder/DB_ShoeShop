...
public class JdbcModelRepository implements ModelRepository {
    ...
    private void insert(Model model) {
        try (PreparedStatement statement = connection.prepareStatement("INSERT INTO MODEL (ID_MODEL) VALUES (?, ?)", Statement.RETURN_GENERATED_KEYS)) {
    ...
    private void update(Model model) {
        try (PreparedStatement statement = connection.prepareStatement("UPDATE MODEL SET NAME = ?, BRAND = ? WHERE ID = ?", Statement.RETURN_GENERATED_KEYS)) {
            statement.setString(1, model.getName());
            statement.setString(2, model.getBrand());
    ...
    public void delete(Model model) {
        try (PreparedStatement statement = connection.prepareStatement("DELETE FROM MODEL WHERE ID = ?")) {
    ...
    public Model get(Integer id) {
        try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM MODEL WHERE ID = ?")) {
            Model model = null;
